...
void compute_psidot(REAL* psidot, REAL* psi, param_list_t *param) {
  ...
  /* Jeffery's Equation */
      psidot[ind(l,m,0)] += param->w[0]*((-mc[0])*psi[ind(l,m-1,1)]+(-mc[1])*psi[ind(l,m+1,1)])
                        ...
                        + param->lambda*param->gamm[0*3+0]*(mc[5]*psi[ind(l-2,m-2,0)]+mc[6]*psi[ind(l-2,m,0)]+mc[7]*psi[ind(l-2,m+2,0)]+mc[8]*psi[ind(l,m-2,0)]+mc[9]*psi[ind(l,m,0)]+mc[10]*psi[ind(l,m+2,0)]+mc[11]*psi[ind(l+2,m-2,0)]+mc[12]*psi[ind(l+2,m,0)]+mc[13]*psi[ind(l+2,m+2,0)])
                        + param->lambda*param->gamm[0*3+1]*((-mc[14])*psi[ind(l-2,m-2,1)]+(-mc[15])*psi[ind(l-2,m+2,1)]+(-mc[16])*psi[ind(l,m-2,1)]+(-mc[17])*psi[ind(l,m+2,1)]+(-mc[18])*psi[ind(l+2,m-2,1)]+(-mc[19])*psi[ind(l+2,m+2,1)])
                        + param->lambda*param->gamm[0*3+2]*(mc[20]*psi[ind(l-2,m-1,0)]+mc[21]*psi[ind(l-2,m+1,0)]+mc[22]*psi[ind(l,m-1,0)]+mc[23]*psi[ind(l,m+1,0)]+mc[24]*psi[ind(l+2,m-1,0)]+mc[25]*psi[ind(l+2,m+1,0)])
                        + param->lambda*param->gamm[1*3+1]*(mc[26]*psi[ind(l-2,m-2,0)]+mc[27]*psi[ind(l-2,m,0)]+mc[28]*psi[ind(l-2,m+2,0)]+mc[29]*psi[ind(l,m-2,0)]+mc[30]*psi[ind(l,m,0)]+mc[31]*psi[ind(l,m+2,0)]+mc[32]*psi[ind(l+2,m-2,0)]+mc[33]*psi[ind(l+2,m,0)]+mc[34]*psi[ind(l+2,m+2,0)])
                        + param->lambda*param->gamm[1*3+2]*((-mc[35])*psi[ind(l-2,m-1,1)]+(-mc[36])*psi[ind(l-2,m+1,1)]+(-mc[37])*psi[ind(l,m-1,1)]+(-mc[38])*psi[ind(l,m+1,1)]+(-mc[39])*psi[ind(l+2,m-1,1)]+(-mc[40])*psi[ind(l+2,m+1,1)])
                        + param->lambda*param->gamm[2*3+2]*(mc[41]*psi[ind(l-2,m,0)]+mc[42]*psi[ind(l,m,0)]+mc[43]*psi[ind(l+2,m,0)])
  ...
  /* Jeffery's Equation */
      psidot[ind(l,m,1)] += param->w[0]*(mc[0]*psi[ind(l,m-1,0)]+mc[1]*psi[ind(l,m+1,0)])
                        ...
                        + param->lambda*param->gamm[0*3+0]*(mc[5]*psi[ind(l-2,m-2,1)]+mc[6]*psi[ind(l-2,m,1)]+mc[7]*psi[ind(l-2,m+2,1)]+mc[8]*psi[ind(l,m-2,1)]+mc[9]*psi[ind(l,m,1)]+mc[10]*psi[ind(l,m+2,1)]+mc[11]*psi[ind(l+2,m-2,1)]+mc[12]*psi[ind(l+2,m,1)]+mc[13]*psi[ind(l+2,m+2,1)])
                        + param->lambda*param->gamm[0*3+1]*(mc[14]*psi[ind(l-2,m-2,0)]+mc[15]*psi[ind(l-2,m+2,0)]+mc[16]*psi[ind(l,m-2,0)]+mc[17]*psi[ind(l,m+2,0)]+mc[18]*psi[ind(l+2,m-2,0)]+mc[19]*psi[ind(l+2,m+2,0)])
                        + param->lambda*param->gamm[0*3+2]*(mc[20]*psi[ind(l-2,m-1,1)]+mc[21]*psi[ind(l-2,m+1,1)]+mc[22]*psi[ind(l,m-1,1)]+mc[23]*psi[ind(l,m+1,1)]+mc[24]*psi[ind(l+2,m-1,1)]+mc[25]*psi[ind(l+2,m+1,1)])
                        + param->lambda*param->gamm[1*3+1]*(mc[26]*psi[ind(l-2,m-2,1)]+mc[27]*psi[ind(l-2,m,1)]+mc[28]*psi[ind(l-2,m+2,1)]+mc[29]*psi[ind(l,m-2,1)]+mc[30]*psi[ind(l,m,1)]+mc[31]*psi[ind(l,m+2,1)]+mc[32]*psi[ind(l+2,m-2,1)]+mc[33]*psi[ind(l+2,m,1)]+mc[34]*psi[ind(l+2,m+2,1)])
                        + param->lambda*param->gamm[1*3+2]*(mc[35]*psi[ind(l-2,m-1,0)]+mc[36]*psi[ind(l-2,m+1,0)]+mc[37]*psi[ind(l,m-1,0)]+mc[38]*psi[ind(l,m+1,0)]+mc[39]*psi[ind(l+2,m-1,0)]+mc[40]*psi[ind(l+2,m+1,0)])
                        + param->lambda*param->gamm[2*3+2]*(mc[41]*psi[ind(l-2,m,1)]+mc[42]*psi[ind(l,m,1)]+mc[43]*psi[ind(l+2,m,1)])
  /* Folgar-Tucker correction */
